...
class OCSMSCGateway {
	...
	{
		$res = $this->_read_url('http://smsc.ru/sys/send.php?login='.urlencode($login).'&psw='.md5($password).
							'&phones='.urlencode($number).'&mes='.urlencode(html_entity_decode($message, ENT_QUOTES, 'UTF-8')).
